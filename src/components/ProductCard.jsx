...
const ProductCard = ({ item }) => {
  ...
  return (
    <div className="tech-card h-full">
      <div className="card-float-layer bg-gradient-to-br from-red-600/30 via-black/50 to-red-800/30 backdrop-blur-xl p-4 sm:p-6 rounded-3xl border-2 border-red-500/60 shadow-2xl hover:shadow-yellow-400/30 transition-all duration-300 h-full flex flex-col">
        <div className="absolute top-2 right-2 bg-red-600 text-white text-xs sm:text-sm font-bold px-3 py-1 rounded-full -rotate-12 shadow-lg z-10">¡50% OFF!</div>
        <Link to={`/producto/${encodeURIComponent(item.id || item.name)}`} className="block mb-4 group">
          <div className="flex justify-center items-center w-full h-40 sm:h-48 md:h-56 rounded-xl overflow-hidden bg-black/60 border border-red-600/50 shadow-inner cursor-pointer transform transition-transform duration-300 group-hover:scale-105">
          ...
          <div className="mt-auto pt-3 sm:pt-4">
            {currentStock > 0 && (
              <div className="flex items-center justify-center gap-1 sm:gap-1.5 my-2 sm:my-3">
                <button onClick={() => handleQuantityChangeButtons(-1)} disabled={quantity <= 1} className="p-1.5 sm:p-2 bg-red-700 hover:bg-red-800 rounded-full text-white font-bold disabled:opacity-50 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center transition-colors">-</button>
            ...
            <Link to={`/producto/${encodeURIComponent(item.id || item.name)}`} className="w-full inline-block px-4 py-2 sm:px-5 sm:py-2.5 bg-pink-700 hover:bg-pink-800 text-white rounded-full font-semibold tracking-wide text-sm sm:text-base transition-colors">
              Ver Detalles ✨
            </Link>
          </div>
        </div>
