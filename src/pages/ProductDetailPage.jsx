...
const ProductDetailPage = () => {
    ...
    const [quantity, setQuantity] = useState(1); // Puede ser string '' temporalmente
    ...
    }, [productName, isLoadingProducts, allProductsWithStock]);
    ...
    useEffect(() => {
        if (product) {
            ...
            if (product.pricingTiers && product.pricingTiers.length > 0) {
                const tierForCurrentQuantity = product.pricingTiers.slice().reverse().find(tier => currentQuantityForCalc >= tier.quantity) || product.pricingTiers[0];
    ...
    const handleDirectQuantityInputChange = (e) => {
        ...
        if (isNaN(newQuantity)) {
            ...
            if (currentStock > 0 && newQuantity > currentStock) {
                setNotification(`Solo quedan ${currentStock} unidades de ${product.name}.`);
    ...
    const handleQuantityInputBlur = () => {
        let finalQuantity = typeof quantity === 'string' && quantity.trim() === '' ? 1 : parseInt(quantity, 10);
        ...
        if (currentStock === 0 && finalQuantity > 0) {
            finalQuantity = 1;
        }
    ...
    const handleQuantityChangeButtons = (amount) => {
        setQuantity(prevQuantity => {
            const currentVal = (typeof prevQuantity === 'number' && !isNaN(prevQuantity)) ? prevQuantity : 1;
    ...
    const handleAddToCart = () => {
        if (product) {
            ...
            if (finalQuantity <= 0) {
                setNotification(`Por favor, selecciona una cantidad válida para ${product.name}.`);
    ...
    if (isLoadingProducts && !product) { return <div className="min-h-screen flex items-center justify-center bg-black text-white text-xl">Cargando producto...</div>; }
    if (!product) { return <div className="min-h-screen flex items-center justify-center bg-black text-white text-xl">Producto no encontrado.</div>; }
    ...
    const displayQuantityForTotalPage = (typeof quantity === 'number' && !isNaN(quantity) && quantity > 0) ? quantity : 1;
            ...
            <div className="max-w-6xl mx-auto py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start mb-12 md:mb-16">
                    ...
                    <div className="flex flex-col space-y-4 md:space-y-5">
                        ...
                        {product.pricingTiers && product.pricingTiers.find(tier => tier.quantity > 1) && (<p className="text-sm text-yellow-400 -mt-2 mb-2">Descuentos por volumen disponibles. Añade más para ver el precio actualizado.</p>)}
                        ...
                        {currentStock > 0 && (
                            <div className="pt-4 space-y-5">
                                ...
                                <button
                                    ...
                                    disabled={currentStock <= 0 || quantity === '' || (typeof quantity === 'number' && (quantity <= 0 || quantity > currentStock))}
                                ...
                                >
                                    {currentStock > 0 ? `Añadir ${quantity || 1} al Carrito (${formatMXN(currentTotal)})` : 'Producto Agotado'}
                                </button>
                            </div>
                        )}
                    </div>
